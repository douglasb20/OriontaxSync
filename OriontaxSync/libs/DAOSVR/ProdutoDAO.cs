...
namespace OriontaxSync.libs.DAOSVR
{
    internal class ProdutoDAO : DefaultModel
    {
        ...
        public List<ProdutosBanco> GetProdctsToSend()
        {
            try
            {
                string query = "SELECT " +
                    ...
                    ", IIF((P.Codigo_EAN LIKE '%.%' OR P.Codigo_EAN LIKE '%,%' OR LEN(P.Codigo_EAN) < 8 OR P.Codigo_EAN LIKE '%-%'),'',P.Codigo_EAN) [ean]" + ", P.NCM[ncm]" + ", P.NOME [descricao]" +
                    ...
                    "  AND CIOU.UF = 'GO' and ciou.status = 0" +
                    "  AND CIO.Cfop__Codigo like '5%'" +
                    "  AND LEN (CIO.icmscst) < 3" +
                    "  AND P.Inativo != 1" + "  AND P.Bloqueado != 1" +
